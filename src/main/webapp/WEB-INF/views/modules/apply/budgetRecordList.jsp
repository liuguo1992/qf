...
<head>
	...
	<script type="text/javascript">
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/apply/budget/_list");
			$("#searchForm").submit();
	    	return false;
	    }
...
<body>
	...
	<form:form id="searchForm" modelAttribute="budgetRecord" action="${ctx}/apply/budget/_list" method="post" class="breadcrumb form-search ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="budgetRecord" varStatus="index">
			<tr>
				...
				<td style="text-align: center;">${budgetRecord.budget.money}</td>
				<td style="text-align: center;">
					<fmt:formatDate value="${budgetRecord.budget.startDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td style="text-align: center;">
					<c:choose>
						<c:when test="${budgetRecord.budget.bStatus  eq 0}" >未完成</font></c:when>
						<c:when test="${budgetRecord.budget.bStatus  eq 1}">已完成</c:when>
					</c:choose>
				</td>
				<td style="text-align: center;">${budgetRecord.budget.bNote}</td>
				<shiro:hasPermission name="apply:budget:edit"><td style="text-align: center;">
					<c:if test="${budgetRecord.status == 0}"><a href="${ctx}/apply/budget/handle?id=${budgetRecord.id}">办理</a></c:if>
					<c:if test="${budgetRecord.status == 1}">
						<a href="${ctx}/apply/budget/delete?id=${budgetRecord.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a>
					</c:if>
