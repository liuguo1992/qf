package com.qFun.qFun.modules.attendance.utils;
...
public class ZkemSDK 
{
	...
	public boolean readTimeGLogData(Date sTime,Date eTime){
		Variant v0 = new Variant(1);
		Variant dwsTime = new Variant(sTime,true);
		Variant dweTime = new Variant(eTime,true);
		
		boolean result = zkem.invoke("ReadTimeGLogData",v0,dwsTime,dweTime).getBoolean();
		return result;
	...
	public List<Map<String,Object>> getUserInfo() 
	{
		...
		{	
			...
			//只支持2位、3位、4位长度的中文名字。
			String name = "";
			byte[] _name = sName.getStringRef().getBytes();
			i++;
			System.out.println("----长度"+_name.length+"名字"+sName.getStringRef()+"-------"+i);
			if(sName.getStringRef().getBytes().length == 9 || sName.getStringRef().getBytes().length == 16)
			{
				name = sName.getStringRef().substring(0,3);
			}else if(sName.getStringRef().getBytes().length == 7 || sName.getStringRef().getBytes().length == 10)
			{
				name = sName.getStringRef().substring(0,2);
			}else if(sName.getStringRef().getBytes().length == 11 || sName.getStringRef().getBytes().length == 22)
			{
